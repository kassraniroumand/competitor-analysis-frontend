...
import {
  ...
  CheckCircle, ShieldAlert, BarChart3
} from "lucide-react";
...
export function ReportPageContent({ ideaId }: { ideaId?: string } = {}) {
  const report = (ideaId && mockReports.find((r) => r.id === ideaId)) || mockReports[0];

  return (
    <div className="p-4 lg:p-8 max-w-7xl mx-auto space-y-4 lg:space-y-6">
        <IdeaBreadcrumb currentPage="Report" />
        <PageHeader
